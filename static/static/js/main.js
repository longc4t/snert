...
layui.use(['element', 'layer', 'layedit', 'laypage'], function () {
    ...
    if (Cookies.get('token')) {
        ...
        articleedit = layedit.build('demo', {
            tool: [
                'strong' //加粗
                , 'italic' //斜体
                , 'underline' //下划线
...
function getdetail() {
    if (window.location.href.indexOf("details") != -1) {
        ...
        $.post("/api/article/search", JSON.stringify({
            "articleid": articleid,
            "token": Cookies.get("token")
        }), function (data) {
            if (data.success) {
...
function uploadarticle() {
    ...
    $.post("/api/article/add", JSON.stringify({
        "articletitle": btoa(title),
        "articleauthor": "",
        "articlecontent": btoa(content),
